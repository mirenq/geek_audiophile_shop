...
/* -------------------------
   ...
   - Use `any` for payload and items safely
   - Runtime checks with Array.isArray
   ------------------------- */
...
function ChartTooltipContent({
  ...
  // Ensure safe typed payload
  const safePayload = Array.isArray(payload) ? (payload as any[]) : [];
    ...
    >
      ...
      <div className="grid gap-1.5">
        {safePayload.map((item: any, index: number) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          // item.payload might be undefined; guard it
